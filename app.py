...
with simulacion_tab:
    ...
    with col1_paciente:
        ...
        opcion_apache: int = st.number_input(label="Apache", min_value=APACHE_MIN, max_value=APACHE_MAX,
                                             value=APACHE_DEFAULT, help=HELP_MSG_APACHE)
        opcion_tiempo_vam: int = st.number_input(label="Tiempo de Ventilación Artificial", min_value=T_VAM_MIN,
                                                 max_value=T_VAM_MAX, value=T_VAM_DEFAULT)
        opcion_estad_uti: int = st.number_input(label="Estadía UCI", min_value=ESTAD_UTI_MIN, max_value=ESTAD_UTI_MAX,
                                                value=ESTAD_UTI_DEFAULT, help=HELP_MSG_ESTAD_UTI)
    with col2_paciente:
        opcion_insuf_resp: str = st.selectbox(label="Tipo Insuficiencia Respiratoria",
                                              options=tuple(INSUF_RESP.values()), index=1)
        opcion_tipo_vam: str = st.selectbox(label="Tipo de Ventilación Artificial", options=tuple(TIPO_VENT.values()), )
        opcion_estad_preuti: int = st.number_input(label="Estadía Pre-UCI", min_value=ESTAD_PREUTI_MIN,
                                                   max_value=ESTAD_PREUTI_MAX, value=ESTAD_PREUTI_DEFAULT,
                                                   help=HELP_MSG_ESTAD_PREUTI)
        input_porciento = st.number_input(label="Porciento Tiempo UCI", min_value=PORCIENTO_SIM_MIN,
                                          max_value=PORCIENTO_SIM_MAX, value=PORCIENTO_SIM_DEFAULT,
                                          help=HELP_MSG_PORCIENTO_SIM)
    with col3_paciente:
        opcion_diagn1: str = st.selectbox(label="Diagnóstico 1", options=tuple(DIAG_PREUCI.values()), index=0)
        opcion_diagn2: str = st.selectbox(label="Diagnóstico 2", options=tuple(DIAG_PREUCI.values()), index=0)
        opcion_diagn3: str = st.selectbox(label="Diagnóstico 3", options=tuple(DIAG_PREUCI.values()), index=0)
        opcion_diagn4: str = st.selectbox(label="Diagnóstico 4", options=tuple(DIAG_PREUCI.values()), index=0)
    ...
    if not st.session_state.df_resultado.empty:
        toggle_format = st.toggle("Formato de tiempo", value=True)
        if toggle_format:
            df_simulacion = format_df_time(build_df_stats(df=st.session_state.df_resultado,
                                                          sample_size=corridas_sim, include_info_label=True))
        else:
            df_simulacion = build_df_stats(df=st.session_state.df_resultado, sample_size=corridas_sim,
        ...
        if diag_ok and insuf_ok:
            try:
                ...
                resultado_experimento = start_experiment(corridas_sim, edad, diagn1, diagn2, diagn3, diagn4, apache,
                                                         ...
                                                         estadia_preuti, input_porciento)
...
with datos_reales_tab:
    ...
    if df_selection:
        st.markdown("Indicadores estadísticos a partir de Paciente seleccionado")
        t: tuple[float] = extract_real_data(RUTA_FICHERODEDATOS_CSV, index=df_selection, return_type="tuple")
        e = start_experiment(
            corridas_simulacion=corridas_sim,
            edad=int(t[0]), d1=t[1], d2=t[2], d3=t[3], d4=t[4],
            apache=t[5], insuf_resp=t[6], va=t[7], est_uci=t[8], t_vam=t[9], est_preuti=t[10],
            porciento=input_porciento
        )
        # DataFrame con experimento con datos reales
        df_sim_datos_reales = build_df_stats(e, corridas_sim, include_info_label=True)
        st.dataframe(
            df_sim_datos_reales,
            hide_index=True,
            use_container_width=True,
