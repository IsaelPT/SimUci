...
def test_coverage_count():
    ...
    vals = {col: 10 for col in VARIABLES_EXPERIMENTO}
    df = make_sim_df(vals, n=100)

    # Llamar a build_df_for_stats solicitando intervalos y métricas
    df_out = build_df_for_stats(df, sample_size=100, include_mean=True, include_std=True, include_confint=True, include_metrics=True, include_info_label=False)
...
def test_coverage_percentage():
    ...
    df_out = build_df_for_stats(df, sample_size=50, include_mean=True, include_std=True, include_confint=True, include_metrics=True, include_info_label=False, metrics_as_percentage=True)
    metric_row = df_out.iloc[-1]
...
def test_metrics_reference_scalar():
    ...
    ref = pd.Series({col: 10 for col in VARIABLES_EXPERIMENTO})

    df_out = build_df_for_stats(df, sample_size=20, include_mean=True, include_std=True, include_confint=True, include_metrics=True, include_info_label=False, metrics_reference=ref)
    metric_row = df_out.iloc[-1]
