"""
Módulo para el procesamiento de datos de pacientes UCI.

Este módulo proporciona funciones para cargar y procesar datos de pacientes
de la Unidad de Cuidados Intensivos (UCI), incluyendo fechas de ingreso/egreso,
diagnósticos, y estadísticas de estancia.
"""
...
def cargar_fichero(path: str, column: str) -> list:
    """
    Carga y retorna una columna específica del archivo CSV, organizada por fecha de ingreso.

    Args:
        path (str): Ruta al archivo CSV de datos
        column (str): Nombre de la columna a extraer

    Returns:
        list: Lista ordenada de los valores de la columna especificada

    Note:
        El archivo CSV debe contener las columnas: fecha_ingreso, fecha_egreso,
        fecha_ing_uci, fecha_egr_uci, tiempo_vam, y diagnostico_preuci
    """
    df = pd.read_csv(
        path,
        index_col=0,
...
def get_fecha_ingreso(path: str):
    """
    Generador de las fechas de ingreso de pacientes.

    Args:
        path (str): Ruta al archivo CSV de datos

    Yields:
        tuple[datetime, datetime]: Tupla conteniendo (fecha_siguiente, fecha_actual)
    """
